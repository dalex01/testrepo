...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    //Modified by Kay Lee
    //move and calculate variables outside of the loop to avoid redundant calculations 
    //Replace document.querySelectorAll with document.getElementsByClassName
    var newPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(newPizzaContainer[0], size);
    var newwidth = (newPizzaContainer[0].offsetWidth + dx) + 'px';
    var docLength =newPizzaContainer.length;
...
//Modified by Kay Lee
//move pizzasDiv to outside of the loop to avoid redundant calculations
//reduce the size of appendChild from 200 to 48
...
function updatePositions() {
  ...
  //Modified by Kay lee
  //replaced to getElementsByClassName() 
  var items = document.getElementsByClassName('mover');
  //Modified by Kay Lee
  //create new local variables to hold static values in order to avoid redundant calculations 
  //define the array's length property at the loop initialization.
  var yTop = document.body.scrollTop/1250;
  var phase0 = Math.sin(yTop);
  var phase1 = Math.sin(yTop+1);
  var phase2 = Math.sin(yTop+2);
  var phase3 = Math.sin(yTop+3);
  var phase4 = Math.sin(yTop+4);  
  
  for (var i = 0, itemLength = items.length; i < itemLength; i++) {
    //var phase = Math.sin((document.body.scrollTop / 1250) + (i % 5));
    //var phase = Math.sin((yTop) + (i % 5));
    //Modified by Kay Lee
    //simplify computations by replacing redundant calculations with the computed static variables
    var iPosition = i % 5;
    ...
    switch(iPosition) {
        ...
        case 3:
             ...
             break;
        case 4:
             items[i].style.left = items[i].basicLeft + 100 * phase4 + 'px';
             break;
        default:
          console.log("bug in position Switcher");
      }
 }
...
//Modified by Kay Lee
//Calculate the number of the elements needed for pre-loading  
  var rows=Math.floor(window.screen.height/s);
  var elementCount = rows * cols;

//Modified by Kay Lee
//Move the variable outside of the loop 
  var elem;
